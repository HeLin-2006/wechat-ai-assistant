 ...
 * <ul>
 *   <li>文本对话 / 图片理解（视觉）：POST {base}/chat/completions</li>
 *   <li>图片生成：POST {base}/images/generations</li>
 *   <li>语音合成：POST {base}/audio/speech</li>
 * </ul>
 */
@Component
public class LlmClient {
     ...
     * 文本对话 / 图片理解。
     *
     * @param messages 完整消息列表（含最新的 user 消息，可按需携带历史）
     * @param imageDataUri 可选，形如 data:image/jpeg;base64,xxx，附加到最后一条 user 消息上
     * @return 模型回复文本
     */
    public String chat(List<ChatMessage> messages, String imageDataUri) {
    ...
    public byte[] generateImage(String prompt) {
        ...
        String json = post("/images/generations", body);
        try {
    ...
    private String post(String path, Object body) {
        try {
            return rest.post().uri(path).body(body).retrieve().body(String.class);
        } catch (RestClientResponseException e) {
            throw new LlmException(
